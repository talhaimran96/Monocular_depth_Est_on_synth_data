...
class Depth_dataset(Dataset):
    def __init__(self, root_directory, dataset='train', transformations=None, mean=[0.485, 0.456, 0.406],
        ...
        if dataset == 'train':
            ...
            self.image_paths = list(pd.read_csv(f"{self.root_directory}/validation.csv")["Images"])
            self.depth_maps = list(pd.read_csv(f"{self.root_directory}/validation.csv")["Depth_maps"])
