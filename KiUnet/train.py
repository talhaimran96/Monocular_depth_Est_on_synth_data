from KiUNetpytorch.arch.ae import kiunet
import torch
...
epochs = 12
starting_epoch = 0
inital_learing_rate = 0.001
random_seed = 42
model = kiunet()
model.soft = torch.nn.Sigmoid() # Replacing the output activation function
model.encoder1 = torch.nn.Conv2d(in_channels=3, out_channels=16, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)) #changing 1st encoder stage to handle 3 channel input
model.final = torch.nn.Conv2d(in_channels=8, out_channels=1, kernel_size=(1, 1), stride=(1, 1)) #changing the out to be single channel
model.encoderf1 = torch.nn.Conv2d(in_channels=3, out_channels=16, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)) #changing 2nd encoder stage to handle 3 channel input
...
run = wandb.init(
    project="Thesis-1",
    config={
        "Model": f"SMP_DEEPLabV3Plus_{encoder_stage}",
...
for epoch in range(starting_epoch, epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(),
                   ...
                   'best_depth_model_SMP_DEEPLabV3Plus_standard_rmse.pth')
